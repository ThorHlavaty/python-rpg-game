...
class Character(object):
    ...
    def attack(self, enemy):
        ...
        if attack_roll - self.dam_mod - self.proficency == 20:
            ...
            enemy.receive_damage((random.randint(
                1, self.power)) + (random.randint(1, self.power)) + (self.dam_mod))
            time.sleep(1.5)
        elif attack_roll >= enemy.ac:
            enemy.receive_damage(
                (random.randint(1, self.power)) + (self.dam_mod), attack_roll)
            time.sleep(1.5)
        else:
    ...
    def throw_torch(self, enemy):
        if self.has_torch == False:
            ...
            print(
                f"You hurl your torch at the {enemy.name}!\nThey catch ablaze!")
            enemy.on_fire = True
...
class Fighter(Character):
    ...
    def attack(self, enemy):
        if self.is_poisoned:
            print(
                f"Poison courses through the {self.name}'s veins, dealing 1 damage!")
            self.hp -= 1
        if not self.alive():
            return
        while True:
            what_to_do = input(
                "Would you like to use a [1]swift attack, or a [2]bold one? ")
            if what_to_do == "1":
                print("%s attacks %s" % (self.name, enemy.name))
                attack_roll = random.randint(
                    1, 20) + self.dam_mod + self.proficency
                print(f"The {self.name} rolls a {attack_roll}!")
                if attack_roll - self.dam_mod - self.proficency > 18:
                    print("Wow! Critical hit!")
                    enemy.receive_damage((random.randint(
                        1, self.power)) + (random.randint(1, self.power)) + (self.dam_mod), attack_roll)
                    time.sleep(1.5)
                elif attack_roll >= enemy.ac:
                    enemy.receive_damage(
                        (random.randint(1, self.power)) + (self.dam_mod), attack_roll)
            ...
            if what_to_do == "2":
                print(
                    f"The figheter lauches a savage attack at the {enemy.name}.")
                attack_roll = random.randint(
                    1, 20) + self.dam_mod + self.proficency
                print(f"The {self.name} rolls a {attack_roll}!")
                if attack_roll - self.dam_mod - self.proficency > 18:
...
class Rogue(Character):
    ...
    def attack(self, enemy):
        ...
        if self.turn_counter == 0:
            ...
            if attack_roll - self.dam_mod - self.proficency == 20:
                print("Wow! Critical hit!")
                enemy.receive_damage((random.randint(1, self.power)) + (random.randint(1, self.power)) + (
                    random.randint(1, self.power)) + (random.randint(1, self.power)) + self.dam_mod, attack_roll)
                time.sleep(1.5)
                self.turn_counter += 1
                if enemy.hp < 1:
                    self.turn_counter = 0
            ...
            while True:
                what_to_do = input(
                    "Would you like to [1]attack, or [2]focus? ")
                if what_to_do == "1":
        ...
        while True:
            if self.spells == 0:
                ...
                if attack_roll - self.dam_mod - self.proficency == 20:
                    ...
                    break
            what_to_do = input(
                "Would you like to use your [1]dagger, or a [2]spell? ")
            if what_to_do == "1":
                print("%s attacks %s" % (self.name, enemy.name))
                attack_roll = random.randint(
                    1, 20) + self.dam_mod + self.proficency
                ...
                while True:
                    ...
                    if self.spells == 1:
                        spell_select = int(input(
                            f"You have {self.spells} spell slot left. What spell would you like to cast? ")) - 1
                    else:
                        spell_select = int(input(
                            f"You have {self.spells} spell slots left. What spell would you like to cast? ")) - 1
                    if self.spell_list[spell_select] == "Firebolt":
                        print("A beam of fire shoots from your fingertips!")
                        dex_save = enemy.dex + \
                            random.randint(1, 20) - self.spell_power
                        self.spells -= 1
                        print(f"The {enemy.name} rolls a {dex_save}")
                        if dex_save > 12 and dex_save < 15:
                            damage_roll = random.randint(
                                1, 6) + self.spell_power
                            print(
                                f"The {enemy.name} barely jumps out of the way! You singe them, dealing {damage_roll}.")
                            ...
                            damage_roll = random.randint(
                                1, 6) + random.randint(1, 6) + self.spell_power
                            print(
                                f"The {enemy.name} is engulfed in flames, causing {damage_roll} damage!")
                            enemy.hp -= damage_roll
                            enemy.on_fire = True
                            time.sleep(1.5)
                            return
                        ...
                        print(
                            f"The sorcerer attempts to soften the {enemy.name}'s demeanor!")
                        wis_save = enemy.wis + \
                            random.randint(1, 20) - self.spell_power
    ...
    def receive_damage(self, points, attack_roll):
        if self.ac + 4 >= attack_roll and self.spells > 0:
            while True:
                print(attack_roll)
                dodge_choice = input(
                    "Would you like to use a spell slot to dodge this attack? [y/n] ")
                if dodge_choice == "y":
...
class Goblin(Character):
    ...
    def attack(self, hero):
        ...
        if self.hp < 4:
            ...
            if attack_roll - self.dam_mod - self.proficency == 20:
                print("Wow! A critical hit!")
                hero.receive_damage((random.randint(
                    1, self.power)) + (random.randint(1, self.power)) + (self.dam_mod) + 2, attack_roll)
                print(f"You took 2 extra damage from the Goblin's furious strike!")
                time.sleep(1.5)
            elif attack_roll >= enemy.ac:
                hero.receive_damage(random.randint(
                    1, 6) + self.dam_mod + 2, attack_roll)
                print(f"You took 2 extra damage from the Goblin's furious strike!")
                time.sleep(1.5)
            else:
                print(f"{self.name.capitalize()} misses!")
    ...
    def attack(self, enemy):
        ...
        if self.turn_counter == 0:
            ...
            if self.spells < 1:
                ...
                print("%s attacks %s with his staff!" %
            ...
            if which_attack == 1:
                ...
                print(
                    f"The wizard sends a puddle of acid flying at the {hero.name}!")
                ...
                if dex_save > 14:
                    ...
                    print(
                        f"The {hero.name} barely jumps out of the way! Some acid splashes on him causing {damage_roll} damage!")
                    con_save = hero.con + random.randint(1, 20)
                    if con_save < 12:
                        hero.is_poisoned = True
                        print(f"The {hero.name} feels a rush of poison!")
                    ...
                    damage_roll = random.randint(
                        1, 4) + random.randint(1, 4) + self.spell_power
                    print(
                        f"The {hero.name} is covered in acid, causing {damage_roll} damage!")
                    hero.hp -= damage_roll
                    con_save = hero.con + random.randint(1, 20)
                    if con_save < 12:
                        hero.is_poisoned = True
...
class MoonGrass(object):
    ...
    def apply(self, hero):
        ...
        if hero.spells >= hero.max_spells:
            ...
            hero.spells += 1
            print("The %s's spells are increased to %d." %
                  (hero.name, hero.spells))
            hero.coins -= 5
            return hero.coins
...
class Wand(object):
    ...
    def apply(self, hero):
        ...
        if hero.has_wand == True:
            ...
            hero.spell_power += 1
            hero.has_wand = True
            print("The %s's spell damage is increased by one and they're harder to resist!" % (
                hero.name))
            hero.coins -= 10
